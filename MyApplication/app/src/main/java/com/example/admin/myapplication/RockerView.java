...
public class RockerView extends View {
    ...
    //纪录上次移动位置
    private static double primary_x=0;
    private static double primary_y=0;
    private static int tuo=0;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_POINTER_UP://两只手指中其中一个抬起来时
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        //测试显示x,y位置
       // if (Math.abs((int)(touchPoint.x-primary_x))>5||Math.abs((int)(touchPoint.y-primary_y))>5) return new Point(centerPoint.x,centerPoint.y);
        //将本次位置记录为上次触摸点位置
        //primary_y=touchPoint.y;
       // primary_x=touchPoint.x;
        // 两点距离
